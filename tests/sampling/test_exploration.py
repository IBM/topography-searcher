...
def test_reconverge_minima():
    ...
    coords = StandardCoordinates(ndim=2, bounds=[(-3.0, 3.0), (-2.0, 2.0)])
    ktn = KineticTransitionNetwork()
    ktn.read_network('.camel.perturb')
    camel = Camelback()
...
def test_reconverge_landscape():
    ...
    coords = StandardCoordinates(ndim=2, bounds=[(-3.0, 3.0), (-2.0, 2.0)])
    ktn = KineticTransitionNetwork()
    ktn.read_network('.camel.perturb2')
    camel = Camelback()
    single_ended = HybridEigenvectorFollowing(camel, 1e-5, 50, 5e-1)
    ...
    assert np.all(ktn.get_ts_coords(0, 5) == pytest.approx(np.array([-1.63806757, -0.22867399])))
    assert np.all(ktn.get_ts_coords(0, 1) == pytest.approx(np.array([-1.10920534, 0.76826805])))
    assert np.all(ktn.get_ts_coords(1, 2) == pytest.approx(np.array([6.15560809e-09, -9.98103796e-08])))
    assert np.all(ktn.get_ts_coords(1, 3) == pytest.approx(np.array([1.2960702, 0.60508434])))
    assert np.all(ktn.get_ts_coords(2, 4) == pytest.approx(np.array([ 1.10920525, -0.76826793])))
    assert np.all(ktn.get_ts_coords(2, 5) == pytest.approx(np.array([-1.29606974, -0.60508394])))
    assert np.all(ktn.get_ts_coords(3, 4) == pytest.approx(np.array([1.63806798, 0.22867407])))
