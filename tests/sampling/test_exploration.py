...
def test_run_connection_attempts():
    ...
    assert np.all(ktn.G[0][1][0]['coords'] == pytest.approx(np.array([-1.109205336676489795, 0.7682680961466419323])))
    assert ktn.G[0][1][0]['energy'] == pytest.approx(0.543718600978186)
    assert np.all(ktn.G[0][5][0]['coords'] == pytest.approx(np.array([-1.638068007045644592, -0.228673952850872414])))
...
def test_get_transition_states():
    ...
    assert np.all(np.abs(ktn.G[1][2][0]['coords']) < 1e-6)
    assert ktn.G[1][2][0]['energy'] == pytest.approx(0.0000)
    assert np.all(ktn.G[1][3][0]['coords'] == pytest.approx(np.array([1.296070266636779600, 0.6050843818689891629])))
    assert ktn.G[1][3][0]['energy'] == pytest.approx(2.22947081802986)
    assert np.all(ktn.G[2][4][0]['coords'] == pytest.approx(np.array([1.109205312695322965, -0.7682680828827476160])))
    assert ktn.G[2][4][0]['energy'] == pytest.approx(0.5437186009781854)
    assert np.all(ktn.G[2][5][0]['coords'] == pytest.approx(np.array([-1.2960702652540481, -0.6050844002904648])))
    assert ktn.G[2][5][0]['energy'] == pytest.approx(2.2294708180298604)
    assert np.all(ktn.G[3][4][0]['coords'] == pytest.approx(np.array([1.638067985280741601, 0.2286740494415425706])))
    assert ktn.G[3][4][0]['energy'] == pytest.approx(2.229357197530713)
