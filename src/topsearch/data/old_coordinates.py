...
class AtomicCoordinates:
    ...
    def write_xyz(self, label: str = '') -> None:
        ...
        with open(f'coords{label}.xyz', 'w') as xyz_file:
...
class MolecularCoordinates(AtomicCoordinates):
    ...
    def get_rotatable_dihedrals(self) -> list:
        ...
        # Find rotatable atoms for each depending on if in loop
        self.rotatable_atoms = []
        if not self.rotatable_dihedrals:
            self.rotatable_atoms = []
        else:
            for i in self.rotatable_dihedrals:
                moved_atoms = self.get_movable_atoms([i[1], i[2]])
                self.rotatable_atoms.append(moved_atoms)

    def get_movable_atoms(self, bond: list) -> list:
        """ For a given dihedral return the atoms that should be rotated """

        ring_atoms = nx.cycle_basis(self.reference_bonds)
        ...
        if bond[0] in ring_atoms and bond[1] in ring_atoms:
            neighbours = self.reference_bonds.neighbors(bond[1])
            moved_atoms = []
            ...
            for j in neighbours:
                if j not in ring_atoms:
                    ...
                    bond_network = self.reference_bonds.copy()
                    bond_network.remove_edge(bond[1], j)
                    for k in nx.connected_components(bond_network):
                        if j in k:
                            for l in k:
                                if l != j:
            ...
            bond_network = self.reference_bonds.copy()
            bond_network.remove_edge(bond[0], bond[1])
            for k in nx.connected_components(bond_network):
                if bond[1] in k:
                    moved_atoms = k
        return list(moved_atoms)

    def get_movable_atoms2(self, bond: list, bond_type: str, bond_network: nx.Graph) -> list:
        """ For a given dihedral return the atoms that should be rotated """

        ring_atoms = nx.cycle_basis(bond_network)
        ring_atoms = [x for xs in ring_atoms for x in xs]
        if bond_type == 'dihedral':
            # Both central atoms are in the loop so treat differently
            if bond[0] in ring_atoms and bond[1] in ring_atoms:
            ...
            if bond[0] in ring_atoms and bond[1] in ring_atoms and bond[2] in ring_atoms:
    ...
    def rotate_dihedral(self, bond: list, angle: float,
        ...
        """ Perform rotation about arbitrary axis """
    ...
    def change_bond_lengths(self, bonds: list, step: list, bond_graph) -> None:
        """ Given a list of bond change their bond lengths by step """
        for count, i in enumerate(bonds, 0):
            # get movable atoms
            moved_atoms = self.get_movable_atoms()
    ...
    def change_dihedral_angles(self, bond_dihedrals: list, step: list) -> None:
    ...
    def remove_atom_clashes(self) -> None:
        """ Check for clashes between atoms. Any atom pairs closer than
            0.8*natural_cutoffs can cause a failure for molecular potentials
            so expand each until the desired length """
        ...
        for i in range(self.n_atoms-1):
            for j in range(i+1, self.n_atoms):
                ...
                allowed_bond_length = 0.8*(self.natural_cutoffs[i] + 
                                           self.natural_cutoffs[j])
                # If too short then expand in both direction to av length
                if bond_length < allowed_bond_length:
                    bond_vector = atom2 - atom1
                    bond_expansion = 0.5*(allowed_bond_length - bond_length)
                    self.position[(3*i):(3*i)+3] = atom1 - bond_vector*bond_expansion
                    self.position[(3*j):(3*j)+3] = atom2 + bond_vector*bond_expansion

    def get_bond_angle_info(self):
        """ Take a set of Cartesian coordinates and compute all 
            dihedrals, angles and bond lengths """

        # Perform analysis to get up-to-date angles and dihedrals
        nl = neighborlist.NeighborList(self.natural_cutoffs,
                                       self_interaction=False)
        self.atoms.set_positions(self.position.reshape(self.n_atoms, 3))
        nl.update(self.atoms)
